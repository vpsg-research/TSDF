...
class HiSD_Trainer(nn.Module):
    ...
    def update_D(self, x, y, j_trg):
        """Updates only the discriminator, keeping the generator frozen."""
        this_model = self.models.module if self.multi_gpus else self.models
        ...
        # Ensure generator parameters are frozen
        for p in this_model.gen.parameters():
            p.requires_grad = False
        ...
        # Generate fake samples with the frozen generator
        with torch.no_grad():
            e = this_model.gen.encode(x)
            # Get random style
            batch = x.size(0)
            
            # 随机选择一个属性类型，而不是固定使用第一个
            num_attributes = len(this_model.gen.extractors.models)
            i = random.randint(0, num_attributes - 1)
            
            # 对于给定的属性类型，获取其可能的值数量
            if i == 2:  # HairColor有三个值：black, blond, brown
                safe_j_trg = min(j_trg, 2)  # 确保索引不超过2
            else:       # 其他属性有两个值：with, without
                safe_j_trg = min(j_trg, 1)  # 确保索引不超过1
                
            j = 0  # 源属性值，使用第一个
            s = this_model.gen.extract(x, i)
            
            # Generate target translation
            s_trg = this_model.gen.map(torch.randn(batch, this_model.gen.noise_dim).cuda(), i, safe_j_trg)
            e_trg = this_model.gen.translate(e, s_trg, i)
            x_trg = this_model.gen.decode(e_trg)
            
            # Generate cycle translation
            e_trg_rec = this_model.gen.encode(x_trg)
            s_trg_rec = this_model.gen.extract(x_trg, i)
            e_cyc = this_model.gen.translate(e_trg_rec, s, i)
            x_cyc = this_model.gen.decode(e_cyc)
        
        # Calculate discriminator loss
        loss_dis_adv = this_model.dis.calc_dis_loss_real(x, s, y, i, j) + \
                      this_model.dis.calc_dis_loss_fake_trg(x_trg, s_trg, y, i, safe_j_trg) + \
                      this_model.dis.calc_dis_loss_fake_cyc(x_cyc, s, y, i, j)
