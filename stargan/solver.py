...
class Solver(object):
    ...
    def perform_stargan(self, x_input, original_x, c_org, the_attack, rand_param):
        ...
        for idx, c_trg in enumerate(c_trg_list):
            with torch.no_grad():
                x_real_mod = original_x
                gen_noattack, _, no_attack_middle = self.G(x_real_mod, c_trg)
            # Attacks
            adv_gen, adv_gen_middle = the_attack.get_stargan_features(x_input, c_trg, self.G, rand_param)
    ...
    def test_universal_model_level(self, i, x_real, c_org, perturb, args):
        ...
        # vutils.save_image(x_adv, './adversarial_examples/stargan_adv_imgs_%d.jpg' % i, nrow=10, normalize=True, range=(-1., 1.))
        ...
        for idx, c_trg in enumerate(c_trg_list):
            with torch.no_grad():
                gen_noattack, gen_noattack_feats, _ = self.G(x_real, c_trg)
                gen, gen_feats, _ = self.G(x_adv, c_trg)
                x_fake_list.append(gen)
