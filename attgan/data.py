...
class WatermarkCelebA(data.Dataset):
    """Dataset class for watermarked CelebA dataset with flexible splits."""
    def __init__(self, data_path, attr_path, image_size, mode, selected_attrs):
        super(WatermarkCelebA, self).__init__()
        self.data_path = data_path
        att_list = open(attr_path, 'r', encoding='utf-8').readlines()[1].split()
        atts = [att_list.index(att) + 1 for att in selected_attrs]
        ...
        # Calculate splits based on actual dataset size
        total_images = len(images)
        train_size = int(total_images * 0.8)  # 80% for training
        valid_size = int(total_images * 0.1)  # 10% for validation
        
        if mode == 'train':
            self.images = images[:train_size]
            self.labels = labels[:train_size]
        elif mode == 'valid':
            self.images = images[train_size:train_size+valid_size]
            self.labels = labels[train_size:train_size+valid_size]
        else:  # test
            self.images = images[train_size+valid_size:]
            self.labels = labels[train_size+valid_size:]
