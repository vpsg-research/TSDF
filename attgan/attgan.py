...
class Generator(nn.Module):
    ...
    def forward(self, x, a=None, mode='enc-dec'):
        if mode == 'enc-dec':
            ...
            return self.decode(self.encode(x), a), self.encode(x)
        if mode == 'enc':
...
class AttGAN():
    ...
    def trainD(self, img_a, att_a, att_a_, att_b, att_b_):
        ...
        # Generate fake image by encoding img_a and then decoding with att_b_
        # This matches the logic in trainG for creating fake images.
        with torch.no_grad(): # Ensure no gradients are computed for G during D training step for this part
            zs_a = self.G(img_a, mode='enc')
            img_fake = self.G(zs_a, att_b_, mode='dec')
        img_fake = img_fake.detach() # Detach after generation
